...
class OnnxruntimeSessionHandler implements SessionHandler {
  ...
  constructor(buffer: Uint8Array|ArrayBuffer) {
    this.#inferenceSession = binding;
    // this.#key = encodeTypedArray(buffer); // JL commented this out
    // JL added the following block of code
    if (ArrayBuffer.isView(buffer)) {  // buffer is Uint8Array
      this.#bufferArr = Array.from(buffer);
    } else {  // buffer is ArrayBuffer
      this.#bufferArr = Array.from(new Uint8Array(buffer));
    }
    this.#key = this.#bufferArr.join(","); // JL: for the sake of simplicity, set key to be the joined "stringified" content of the bufferArr array separated by commas
  ...
  async loadModel(options: InferenceSession.SessionOptions): Promise<void> {
    try {
      // load a model
      // const results: Binding.ModelLoadInfoType = await this.#inferenceSession.loadModel(this.#key, options);  // JL commented this out
      const results: Binding.ModelLoadInfoType = await this.#inferenceSession.loadModel(this.#bufferArr, options);  // JL added this line
...
class OnnxruntimeBackend implements Backend {
  ...
  async createSessionHandler(buffer: Uint8Array|ArrayBuffer, options?: InferenceSession.SessionOptions):
      Promise<SessionHandler> {
    const handler = new OnnxruntimeSessionHandler(buffer);
    await handler.loadModel(options || {});
    return handler;
  }
}
