...
public class OnnxruntimeModule extends ReactContextBaseJavaModule {
  ...
  // JL added
  @ReactMethod
  public void loadModel(ReadableArray bufferArr, ReadableMap options, Promise promise) {
    try {
      WritableMap resultMap = loadModel(bufferArr, options);
      promise.resolve(resultMap);
    } catch (Exception e) {
      promise.reject("Can't read a model " + bufferArr, e);
    }
  }
  ...
  // JL added
  public WritableMap loadModel(ReadableArray bufferArr, ReadableMap options) throws Exception {
    byte[] byteArr = new byte[bufferArr.size()];
    String[] strArr = new String[bufferArr.size()];
    for (int i = 0; i < byteArr.length; i++) {
      int num = bufferArr.getInt(i);
      byteArr[i] = (byte)num;
      strArr[i] = String.valueOf(num);
    }

    InputStream modelStream = new ByteArrayInputStream(byteArr);
    String key = String.join(",", strArr); // JL: for the sake of simplicity, set key to be the joined "stringified" content of the array separated by commas
