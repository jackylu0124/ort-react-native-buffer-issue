...
// JL added
const encodeTypedArray = (arr: Uint8Array|ArrayBuffer): string => {
  var arrBuffer: ArrayBuffer;
  if (ArrayBuffer.isView(arr)) {  // arr is Uint8Array
    arrBuffer = arr.buffer;
  } else {  // arr is ArrayBuffer
    arrBuffer = arr;
  }
  return Buffer.from(arrBuffer, 0, arrBuffer.byteLength).toString('base64');
};
...
class OnnxruntimeSessionHandler implements SessionHandler {
  ...
  constructor(buffer: Uint8Array|ArrayBuffer) {
    this.#inferenceSession = binding;
    this.#key = encodeTypedArray(buffer);
    this.inputNames = [];
    this.outputNames = [];
  }
